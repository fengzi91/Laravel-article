...
;(function (global, factory) {
  ( false ? 'undefined' : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? module.exports = factory() :  true ? !(__WEBPACK_AMD_DEFINE_FACTORY__ = (factory),
				...
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__)) : global.mdui = factory();
})(this, function () {
  'use strict';
  ...
  var $ = function (window, document, undefined) {
    ...
    function _map(elems, callback) {
      var value;
      var ret = [];

      _each(elems, function (i, elem) {
    ...
    $.fn.extend({
      ...
      map: function map(callback) {
        return new JQ(_map(this, function (el, i) {
      ...
      get: function get(index) {
        return index === undefined ? _slice.call(this) : this[index >= 0 ? index : index + this.length];
      },
       ...
       * @returns {JQ}
       */
      slice: function slice(argument) {
        return new JQ(_slice.apply(this, arguments));
      },
      ...
      is: function is(selector) {
        ...
        if (isString(selector)) {
          ...
          var matchesSelector = _this.matches || _this.matchesSelector || _this.webkitMatchesSelector || _this.mozMatchesSelector || _this.oMatchesSelector || _this.msMatchesSelector;
      ...
      children: function children(selector) {
        var children = [];
        this.each(function (i, _this) {
          _each(_this.childNodes, function (i, childNode) {
      ...
      serializeArray: function serializeArray() {
        ...
        $(_slice.call(ele.elements)).each(function () {
          $ele = $(this);
          type = $ele.attr('type');
      ...
      serialize: function serialize() {
        var result = [];
        _each(this.serializeArray(), function (i, elm) {
     ...
     * @return {*|JQ}
     */
    _each(['val', 'html', 'text'], function (nameIndex, name) {
      var props = {
        0: 'value',
        1: 'innerHTML',
        2: 'textContent'
     ...
     * @return {String|JQ}
     */
    _each(['attr', 'prop', 'css'], function (nameIndex, name) {
      var set = function set(ele, key, value) {
        if (nameIndex === 0) {
     ...
     * @return {JQ}
     */
    _each(['add', 'remove', 'toggle'], function (nameIndex, name) {
        ...
        if (val === undefined) {
          ...
          if ('ActiveXObject' in window) {
            ...
            if ($ele.css('box-sizing') === 'border-box') {
              IEFixValue = parseFloat($ele.css('padding-' + (isWidth ? 'left' : 'top'))) + parseFloat($ele.css('padding-' + (isWidth ? 'right' : 'bottom'))) + parseFloat($ele.css('border-' + (isWidth ? 'left' : 'top') + '-width')) + parseFloat($ele.css('border-' + (isWidth ? 'right' : 'bottom') + '-width'));
            }
          }
    ...
    var dir = function dir(nodes, selector, nameIndex, node) {
      var ret = [];
      var ele;
     ...
     * @return {JQ}
     */
    _each(['', 'All', 'Until'], function (nameIndex, name) {
     ...
     * @return {JQ}
     */
    _each(['', 's', 'sUntil'], function (nameIndex, name) {
     ...
     * @return {JQ}
     */
    _each(['append', 'prepend'], function (nameIndex, name) {
      $.fn[name] = function (newChild) {
        ...
        if (isString(newChild)) {
          ...
          newChilds = _slice.call(tempDiv.childNodes);
        } else {
        ...
        return this.each(function (i, _this) {
          _each(newChilds, function (j, child) {
     ...
     * @return {JQ}
     */
    _each(['insertBefore', 'insertAfter'], function (nameIndex, name) {
      $.fn[name] = function (selector) {
        var $ele = $(selector);
        return this.each(function (i, _this) {
          $ele.each(function (j, ele) {
            ele.parentNode.insertBefore($ele.length === 1 ? _this : _this.cloneNode(true), nameIndex === 0 ? ele : ele.nextSibling);
          });
        });
      };
    });
    ...
    (function () {
      ...
      $.extend({
         ...
         * @param value 可选，值
         */
        data: function data(ele, key, value) {
          var data = {};
          ...
          if (value !== undefined) {
            ...
            // 获取所有值
            var result = {};
            _each(ele.attributes, function (i, attribute) {
         ...
         * @param key 必须，键名
         */
        removeData: function removeData(ele, key) {
    ...
    (function () {
      ...
      $.fn.extend({
        ...
        on: function on(eventName, selector, data, _callback, one) {
          ...
          if (eventName && !isString(eventName)) {
            _each(eventName, function (type, fn) {
          ...
          if (one === 1) {
            ...
            _callback = function callback() {
              _this.off(eventName, selector, _callback);
      ...
      function add(element, eventName, func, data, selector) {
        ...
        eventName.split(' ').forEach(function (event) {
          ...
          var callFn = function callFn(e, ele) {
    ...
    (function () {
       ...
       * @returns {boolean}
       */
      var isQueryStringData = function isQueryStringData(method) {
      ...
      $.extend({
        ...
        ajax: function ajax(options) {
          ...
          _each(globals, function (key, value) {
          ...
          // 添加 headers
          if (options.headers) {
            _each(options.headers, function (key, value) {
          ...
          if (options.xhrFields) {
            _each(options.xhrFields, function (key, value) {
              xhr[key] = value;
            });
          }
          ...
          xhr.onload = function () {
            ...
            if (isHttpStatusSuccess) {
              ...
              if (options.dataType === 'json') {
                ...
                eventParams.data = responseData = xhr.responseType === 'text' || xhr.responseType === '' ? xhr.responseText : xhr.response;
            ...
            // statusCode
            _each([globals.statusCode, options.statusCode], function (i, func) {
      ...
      _each(ajaxEvent, function (name, eventName) {
        $.fn[name] = function (fn) {
  ...
  (function () {
    ...
    var Ripple = {
      ...
      show: function show(e, $ripple) {
        ...
        // 涟漪的 DOM 结构
        $('<div class="mdui-ripple-wave" style="' + 'width: ' + diameter + 'px; ' + 'height: ' + diameter + 'px; ' + 'margin-top:-' + diameter / 2 + 'px; ' + 'margin-left:-' + diameter / 2 + 'px; ' + 'left:' + center.x + 'px; ' + 'top:' + center.y + 'px;">' + '</div>')
  ...
  (function () {
    ...
    var getProp = function getProp(obj, prop) {
      return (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === 'object' && obj !== null && obj[prop] !== undefined && obj[prop] ? obj[prop] : false;
    };
  ...
  mdui.Select = function () {
    ...
    Select.prototype.handleUpdate = function () {
      ...
      // select
      _this.$select = $('<div class="mdui-select mdui-select-position-' + _this.options.position + '" ' + 'style="' + _this.$selectNative.attr('style') + '" ' + 'id="' + _this.uniqueID + '"></div>').show().append(_this.$selected);
  ...
  mdui.Tooltip = function () {
    ...
    function Tooltip(selector, opts) {
      ...
      // 创建 Tooltip HTML
      _this.$tooltip = $('<div class="mdui-tooltip" id="' + $.guid() + '">' + _this.options.content + '</div>').appendTo(document.body);
  ...
  (function () {
    ...
    function Snackbar(message, opts) {
      ...
      // 添加 HTML
      _this.$snackbar = $('<div class="mdui-snackbar">' + '<div class="mdui-snackbar-text">' + _this.message + '</div>' + (_this.options.buttonText ? '<a href="javascript:void(0)" ' + 'class="mdui-snackbar-action mdui-btn mdui-ripple mdui-ripple-white ' + buttonColorClass + '" ' + buttonColorStyle + '>' + _this.options.buttonText + '</a>' : '') + '</div>').appendTo(document.body);
